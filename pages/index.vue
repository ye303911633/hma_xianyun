...
export default {
  ...
  methods: {
    handleOption(index) {
      this.currentOption = index; // 把当前切换的索引值给到currentOption 切换样式
      /*  const item = this.options[index] // 把当前索引位置的options中的数据赋值到item
      if(item.name === "机票"){
        return this.$router.push(item.pageUrl)  // 如果切换tab的是机票，就根据用户搜索的关键字进行URL跳转
      } */
    },
    handleSearch() {
